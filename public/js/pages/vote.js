...
export function renderVote(container, pollId) {
    teardownStage();
    container.innerHTML = '<div class="vote-hero"><p>Loading...</p></div>';
    loadVote(container, pollId);
}
async function loadVote(container, pollId) {
    try {
        const vh = voterHeaders();
        const { voted } = await api(`/polls/${pollId}/voted`, { headers: vh });
        state.poll = await api(`/polls/view/${pollId}`);
        ...
        const data = await api(`/polls/${pollId}/pairings`, { headers: vh });
        if (data.voterToken)
            storeVoterToken(data.voterToken);

...
async function startPreview(container) {
    ...
    `;
        ...
        bar.innerHTML = pairings.pairings.map((_, i) => `<div class="progress-dot${i < round ? ' done' : ''}${i === round ? ' current' : ''}"></div>`).join('');
...
function loadTabContent(poll) {
    ...
    switch (currentTab) {
        case 'images':
            renderImagesTab(poll);
            break;
        case 'settings':
            renderSettingsTab(poll);
            break;
        case 'size':
            renderSizeTab(poll);
            break;
...
function setupUpload(el, poll) {
    ...
    const syncPoll = async () => {
        const fresh = await api(`/polls/${pollId}`, { headers: authHeaders(token) });
    ...
    const uploadOne = async (file) => {
        ...
        try {
            const img = await api(`/polls/${pollId}/upload`, {
                method: 'POST',
...
function renderSettingsTab(poll) {
...
function renderSizeTab(poll) {
  ...
  `;
    const wInput = document.getElementById('container-w');
    const hInput = document.getElementById('container-h');
    const lockIcon = document.getElementById('lock-icon');
    let fitMode = poll.fitMode;
    let arLocked = false;
    let ar = W / H;
    ...
    panel.addEventListener('click', (e) => {
        ...
        if (target.tagName === 'IMG' && firstImg) {
            openLightbox(target.src, imgLabel(poll.images, firstImg.id));
        }
    });
    const updatePreview = () => {
        ...
        const scroll = document.getElementById('allow-scrolling').checked;
    ...
    document.getElementById('reset-size')?.addEventListener('click', () => {
        ...
        hInput.value = '600';
        fitMode = 'contain';
        document.getElementById('allow-scrolling').checked = false;
        el.querySelectorAll('.size-fit-btn').forEach(b => b.classList.remove('active'));
...
function renderShareTab(_poll) {
  ...
  `;
    ...
    document.getElementById('gen-share').addEventListener('click', async () => {
        try {
            const data = await api(`/polls/${pollId}/share`, {
                headers: authHeaders(token),
            });
...
async function renderMetadataTab() {
    ...
    try {
        const [resData, votersData] = await Promise.all([
            api(`/polls/${pollId}/results`, { headers: authHeaders(token) }),
            api(`/polls/${pollId}/voters`, { headers: authHeaders(token) }),
        ]);
        ...
        const sorted = [...resData.poll.images].sort((a, b) => {
          ...
          <tr class="selections-detail" id="sel-${v.fingerprint}">
            <td colspan="4">
              ${v.selections.map((s, i) => {
                    const leftImg = resData.poll.images.find((img) => img.id === s.leftImageId);
                    const rightImg = resData.poll.images.find((img) => img.id === s.rightImageId);
                    return `
                  <div class="sel-row">
...
function imgIdx(images, imageId) {
    return images.findIndex(i => i.id === imageId) + 1;
}
function imgLabel(images, imageId) {
